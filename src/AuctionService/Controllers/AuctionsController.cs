...
public class AuctionsController : ControllerBase
{
    ...
    public async Task<ActionResult<List<AuctionDto>>> GetAllAuctions(string date = null){
        ...
        if(!string.IsNullOrEmpty(date)){
            query = query.Where(x => x.UpdatedAd.CompareTo(DateTime.Parse(date).ToUniversalTime()) > 0);
        }
    ...
    public async Task<ActionResult<AuctionDto>> CreateAuction(CreateAuctionDto auctionDto){
        ...
        //All one transaction bc all lines part of entity franework, including msg publishing
